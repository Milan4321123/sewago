...
// The counter and the batcher race: a shopkeeper may hand an order over while
// its run is still waiting for a rider (the handover guard only blocks runs a
// courier is actually working). The tests below pin down what the batcher must
// do about it: never offer — or pay for — cargo that already left the shop.
...
test('a run emptied at the counter is cancelled, not re-offered', async () => {
  ...
  // The customer turns up at the shop and the shopkeeper hands the bag over.
  // Allowed — no courier is carrying anything yet.
  const handed = await api(`/partner/store-orders/${order.id}/handover`, { method: 'POST', token: shop.token });
  assert.equal(handed.status, 200, JSON.stringify(handed.data));
...
test('accepting a run whose order was just handed over is refused', async () => {
  ...
  // The customer collects at the counter while the offer card is still on the
  // rider's screen. Legal — the run is only 'offered', nobody is carrying it.
  const handed = await api(`/partner/store-orders/${order.id}/handover`, { method: 'POST', token: shop.token });
  assert.equal(handed.status, 200, JSON.stringify(handed.data));
...
test('a half-handed-over run is re-planned, not paid at full size', async () => {
  // Two cash orders batch while no rider is around; one customer then collects
  // at the counter. When a rider finally comes online, the run they are offered
  // must be the one order still on the shelf — one pickup, one drop, that
  // order's cash — not the original route and payout.
  ...
  const handed = await api(`/partner/store-orders/${o1.id}/handover`, { method: 'POST', token: shop.token });
  assert.equal(handed.status, 200, JSON.stringify(handed.data));
  ...
  assert.ok(
    ...
    'the counter-handed order must not be debited to the rider'
  );
});
